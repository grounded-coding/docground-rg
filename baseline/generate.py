...
from .main import smart_tokenizer_and_embedding_resize
...
def evaluate(args, eval_dataset, model, tokenizer, desc="", accelerator=None, gen_task="seq2seq_lm") -> Dict:
    ...
    for n, batch in enumerate(tqdm(eval_dataloader, desc="Evaluating", disable=False)):
        with torch.no_grad():
        ...
        if ground_truth.strip() != "":
            ...
            if n % 20 == 0:
                logger.debug(f"Decoded the following prediction\n{tokenizer.decode(sampled_output_ids[0], skip_special_tokens=True)}"
                            f"\n for ground truth \n{ground_truth}")
    ...
    if accelerator.is_main_process:
        ...
        # Remove leading white spaces if necessary and assistant token if no special tokens are used
        all_sampled_texts = [[text.lstrip().replace(SPECIAL_TOKENS["additional_special_tokens"][0], "", 1).lstrip() for text in texts] for texts in all_sampled_texts]
