...
train_command="export MAX_DESIRED_LEN=${max_desired_len}; singularity exec --nv${binds} $HOME/setups/apptainer/nlp_torch_sis.sif accelerate launch --mixed_precision=${acc_mp} --config_file ${accelerate_config} --main_process_port=25678 --num_processes=${gpus} baseline.py --params_file ${params_file} --task generation --dataroot data_filtered --knowledge_file knowledge.json --exp_name ${model_alias}-rg-review-${suffix} ${debug_flag} ${debug_fill}"
eval_command="export MAX_DESIRED_LEN=${max_desired_len}; singularity exec --nv${binds} $HOME/setups/apptainer/nlp_torch_sis.sif accelerate launch --mixed_precision=${acc_mp} --config_file ${accelerate_config} --main_process_port=25679 --num_processes=${gpus} baseline.py --generate runs/${model_alias}-rg-review-${suffix} --generation_params_file ${generation_params_file} --task generation --dataroot data_filtered --eval_dataset ${eval_split} --labels_file data/${eval_split}/labels.json --knowledge_file knowledge.json --output_file pred/${eval_split}/rg.${model_alias}-${suffix}.json ${debug_flag} ${debug_fill}"
